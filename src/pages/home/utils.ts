    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        const $container_: HTMLElement | null =
          document.querySelector(containerTagName)
        if ($container_) {
          resolve($container_)
          return
        }
...
export function queryPopupCardElement(): HTMLDivElement | null {
  return document.querySelector(`#${popupCardID}`) as HTMLDivElement | null
}
